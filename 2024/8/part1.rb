...
nodes.each do |letter, points|
  (0..points.count-2).each do |i|
    (i+1..points.count-1).each do |j|
      ...
      new_point_1 = points[j].px(x_diff).py(y_diff)
      new_point_2 = points[i].mx(x_diff).my(y_diff)

      if x_range.include?(new_point_1.x) && y_range.include?(new_point_1.y)
        antinodes << new_point_1
      end

      if x_range.include?(new_point_2.x) && y_range.include?(new_point_2.y)
